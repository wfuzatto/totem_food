...
async function syncGatewayPayment(id){
  ...
  if(remote.status==='APPROVED'){
    ...
    await audit('api_pagamento','PAYMENT_EVENT','order',id,{status:remote.status,external_id:pay.external_id,...(remote.details||{})});
    await finalizePaidOrder(id);
  }else if(['DECLINED','CANCELLED','ERROR'].includes(remote.status)){
    await p.execute("UPDATE orders SET payment_status=?,status='PAYMENT_FAILED' WHERE id=? AND status='PAYMENT_PENDING'",[remote.status,id]);
    await audit('api_pagamento','PAYMENT_EVENT','order',id,{status:remote.status,external_id:pay.external_id,...(remote.details||{})});
  }
  return remote;
}
